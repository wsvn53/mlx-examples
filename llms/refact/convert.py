...
def convert(args):
    ...
    weights = {replace_key(k): (v.numpy() if v.dtype != torch.float16 else v.to(torch.float16).numpy()) for k, v in state_dict.items()}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert Refact-1.6B model to npz")
